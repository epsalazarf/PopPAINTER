        ...
        fluidRow(
          box(width = 8, status = "primary", solidHeader = TRUE,
              ...
              tags$p(tags$strong("Popinfo (.tsv / .txt / .csv):"),
                     ...
                     "metadata such as ", tags$code("POP"), ", ", tags$code("POPULATION"), ", ",
                     tags$code("META"), ", ", tags$code("SUPER"), ", ", tags$code("COUNTRY"),
                     ". These are optional and, when present, expected to follow a loose ",
                     "hierarchy — ", tags$code("POP"), " → ", tags$code("META"), " → ",
                     tags$code("SUPER"), " — from most to least granular (", tags$code("META"),
                     " is typically a linguistic, ethnic, or subcontinental grouping; ",
                     tags$code("SUPER"), " is typically continental-level)."),
              tags$pre(
                "K1        K2        K3\n",
